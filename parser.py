...
def initialize():
    ...
    items = []
    categories = {}
    dump_pkl('users.pkl', users)
    dump_pkl('items.pkl', items)
    dump_pkl('bids.pkl', bids)
    dump_pkl('categories.pkl', categories)
...
def parseJson(json_file):
   ...
   # print(type(categories))
    ...
    # Dictionary? Why not list of dictionary
    ...
    with open(json_file, 'r') as f:
        ...
        # print(f'Number of items in JSON is {l}')
        # l = 3
        for i in range(l):
            curr = json_items[i]
            getUsers(curr, users)
    ...
    # print(len(t))
    # print(t)
    # print(f'Number of items in .dat {len(t)}')
    # for i in range(len(t)):
    #     if t[i] == '':
    #         print(i)

def load_pkl(file_name):
    ''' Unpickles the binary file from memory, unpickle collection of urls to keep track and also the dict for pandas df'''
    with open(file_name,'rb') as file:
        data = pickle.load(file)
    return data

def dump_pkl(file_name, dictionary):
    ''' Dumps dictionary to save progress.'''
